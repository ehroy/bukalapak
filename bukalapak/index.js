...
const get_regist_formdata = (identi, cookieidenti, browser_id, session, bebe, email, nama, authenticity)=> new Promise((resolve,reject)=>{
  fetch("https://accounts.bukalapak.com/register",{
    ...
    body: JSON.stringify({"access_token":"12n9yzQKGbP7yccftqZPlfa8rTxNAoh_dIoDD6KTMJiX6Q","user":{"email":`${email}`,"name":`${nama}`,"password":"kaserinas123","password_confirmation":"kaserinas123"},"authenticity_token":`${authenticity}`})
  })
  .then(res => res.json())
  .then(ress=>{
    resolve(ress)
...
(async()=>{
  // const devicekey = await device_key();
  // const id = devicekey
  // console.log(id)
   ...
   const nomor = readlineSync.question("[REQ] Masukan nomer ? ")
      ...
      const mesagge2 = register_otp_form.data
      console.log(`[INFO] ${mesagge2}`)
   }if(fitur === "2"){
